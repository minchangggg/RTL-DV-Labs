# Basic 8-Bit CPU
_This project implements a tiny 8-bit accumulator CPU with a single unified memory for instructions and data._

_The design is fully synchronous (active-low reset) and runs a simple two-phase Fetch/Execute sequence controlled by a finite-state machine._
...
`SKZ` (skip next if **ACC==0** via Z flag), 

`ADD`, 

`AND`, 

`XOR` (operate on `ACC` with `MEM[addr]`), 
